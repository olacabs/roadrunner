 ...
 */
public class ColumnFilterDoubleNonIndexedImpl implements ColumnFilterDouble {
	...
	private static final Logger logger = LoggerFactory.getLogger(ColumnFilterDoubleNonIndexedImpl.class);
	
	public ColumnFilterDoubleNonIndexedImpl(String[] partitionBucketIds, String fieldName, RecordHolder[][] recordHolders, BitSetExposed[] availablePosBitSets, BitsetCache bitsCache) {
		super();
	...
	private RecordIndexes opwork(Operation operation, double... values) {
	...
	private final int operate(final Operation operation, BitSetExposed intermediateBitSet, final RecordHolder[] recordHolderArr, final String bucketId, final int recordsT,
	        int recordI, final long word, final int bitStartIndex, final int bitEndIndex, final double... values) {
	...
	public void validateDataType(Record record, Object valueObj) {
		...
		if(valueObj instanceof Double) return;
		...
		metrics.increment("searcher_invalid_Double_data_type", 1);
		logger.error("valueObj {}, class : {} is not instance of Double or List of Double"
		...
		throw new CorruptedIndexException("Data type of field Name : " + fieldName 
				+ " is not Double or List of Double for Record Id : " + record.getId());
	}

	private void callbackEqualTo(BitSetExposed intermediateBits, int bitIndex, Object valueObj, double... compareValues) {
		if(valueObj == null) {
		...
		if(valueObj instanceof Double) {
			for(double compareValue : compareValues) {
				if(compareValue == ((Double) valueObj).doubleValue()) {
					valueMatch = true;
					break;
				}
			}
		} else {
			List<Double> values = (List<Double>) valueObj;
		
			for(Double value : values) {
				for(double compareValue : compareValues) {
					if(value != null && compareValue == value.doubleValue()) {
						valueMatch = true;
						break;
					}
				}
	...
	private void callbackGreaterThan(BitSetExposed intermediateBits, int bitIndex, Object valueObj, double compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
		...
		boolean valueMatch = false;
		if(valueObj instanceof Double) {
			if(((Double) valueObj).doubleValue() > compareValue) {
				valueMatch = true;
			}
			...
			for(Double value : values) {
				if(value != null && value.doubleValue() > compareValue) {
					valueMatch = true;
					break;
				}
			}
	...
	private void callbackGreaterThanEqualTo(BitSetExposed intermediateBits, int bitIndex, Object valueObj, double compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
	...
	private void callbackLessThan(BitSetExposed intermediateBits, int bitIndex, Object valueObj, double compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
	...
	private void callbackLessThanEqualTo(BitSetExposed intermediateBits, int bitIndex, Object valueObj, double compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
	...
	private void callbackRange(BitSetExposed intermediateBits, int bitIndex, Object valueObj, double minValue, double maxValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
		...
		boolean valueMatch = false;
		if(valueObj instanceof Double) {
			double value = ((Double) valueObj).doubleValue();
			...
			for(Double value : values) {
				if(value != null) {
					double nativeValue = value.doubleValue();
