 ...
 */
public class ColumnFilterShortNonIndexedImpl implements ColumnFilterShort {
	...
	private static final Logger logger = LoggerFactory.getLogger(ColumnFilterShortNonIndexedImpl.class);
	
	public ColumnFilterShortNonIndexedImpl(String[] partitionBucketIds, String fieldName, RecordHolder[][] recordHolders, BitSetExposed[] availablePosBitSets, BitsetCache bitsCache) {
		super();
	...
	private RecordIndexes opwork(Operation operation, short... values) {
	...
	private final int operate(final Operation operation, BitSetExposed intermediateBitSet, final RecordHolder[] recordHolderArr, final String bucketId, final int recordsT,
	        int recordI, final long word, final int bitStartIndex, final int bitEndIndex, final short... values) {
	...
	public void validateDataType(Record record, Object valueObj) {
		...
		if(valueObj instanceof Short) return;
		...
		metrics.increment("searcher_invalid_Short_data_type", 1);
		logger.error("valueObj {}, class : {} is not instance of Short or List of Short"
		...
		throw new CorruptedIndexException("Data type of field Name : " + fieldName 
				+ " is not Short or List of Short for Record Id : " + record.getId());
	}

	private void callbackEqualTo(BitSetExposed intermediateBits, int bitIndex, Object valueObj, short... compareValues) {
		if(valueObj == null) {
		...
		if(valueObj instanceof Short) {
			for(short compareValue : compareValues) {
				if(compareValue == ((Short) valueObj).shortValue()) {
					valueMatch = true;
					break;
				}
			}
		} else {
			List<Short> values = (List<Short>) valueObj;
		
			for(Short value : values) {
				for(short compareValue : compareValues) {
					if(value != null && compareValue == value.shortValue()) {
						valueMatch = true;
						break;
					}
				}
	...
	private void callbackGreaterThan(BitSetExposed intermediateBits, int bitIndex, Object valueObj, short compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
		...
		boolean valueMatch = false;
		if(valueObj instanceof Short) {
			if(((Short) valueObj).shortValue() > compareValue) {
				valueMatch = true;
			}
			...
			for(Short value : values) {
				if(value != null && value.shortValue() > compareValue) {
					valueMatch = true;
					break;
				}
			}
	...
	private void callbackGreaterThanEqualTo(BitSetExposed intermediateBits, int bitIndex, Object valueObj, short compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
	...
	private void callbackLessThan(BitSetExposed intermediateBits, int bitIndex, Object valueObj, short compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
	...
	private void callbackLessThanEqualTo(BitSetExposed intermediateBits, int bitIndex, Object valueObj, short compareValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
	...
	private void callbackRange(BitSetExposed intermediateBits, int bitIndex, Object valueObj, short minValue, short maxValue) {
		if(valueObj == null) {
			intermediateBits.clear(bitIndex);
			return;
		}
		...
		boolean valueMatch = false;
		if(valueObj instanceof Short) {
			short value = ((Short) valueObj).shortValue();
			...
			for(Short value : values) {
				if(value != null) {
					short nativeValue = value.shortValue();
